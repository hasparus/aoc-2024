    ...
    for (i, line) in lines.iter().enumerate() {
        for (j, &symbol) in line.as_bytes().iter().enumerate() {
            if !visited[i][j] {
                ...
                let (size, perimeter) = explore_region(&lines, &mut visited, i, j, symbol);
                regions.push(Region {
                    symbol,
                    area: size,
                    perimeter,
...
fn main() -> Result<(), std::io::Error> {
    ...
    for input in inputs.sections {
        let regions = flood_fill(&input.content);

        let price = regions.iter().map(|r| r.area * r.perimeter).sum::<usize>();
        println!("{:<10}\t{}", input.name, price);
    }
